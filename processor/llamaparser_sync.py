"""
LlamaParser Synchronous - Parse all PDFs with LlamaParser
Uses synchronous calls to avoid event loop issues
"""
import json
import time
...
class LlamaParserSync:
    """Process documents using LlamaParser API - Synchronous version"""
    ...
    def parse_document(self, file_path: Path, doc_info: dict) -> Optional[ParsedDocument]:
        ...
        try:
            # LlamaParse load_data is synchronous
    ...
    def parse_all_documents(self) -> Dict[str, ParsedDocument]:
        ...
        console.print(f"[blue]Total: {len(pdf_files)} PDF files to parse[/blue]")
...
def main():
    client = LlamaParserSync()
    client.parse_all_documents()
